...
suite(`SyncPgRepository`, () => {
  ...
  suite(`Get list of user not synced`, () => {
    test(`Should error get list of user not synced`, async () => {
      const queryError = new Error('Query error');
      testObj.postgresDb.query.throws(queryError);

      const [error] = await testObj.syncRepository.getListOfUserNotSynced();

      testObj.postgresDb.query.should.have.callCount(1);
      testObj.postgresDb.query.should.have.calledWith(
        sinon.match.has('values', sinon.match.array.deepEquals([SyncModel.SERVICE_SYNC_USER, 3])),
      );
      expect(error).to.be.an.instanceof(DatabaseExecuteException);
      expect(error).to.have.property('httpCode', 400);
      expect(error).to.have.property('isOperation', false);
    ...
    test(`Should successfully list of user not synced`, async () => {
      const fetchQuery = {
        get rowCount() {
          return 0;
        },
      ...
      const [error, result] = await testObj.syncRepository.getListOfUserNotSynced();
    ...
    test(`Should successfully get all list of user not synced`, async () => {
      const fetchQuery = {
        get rowCount() {
          return 1;
        },
          ...
          return [
            {
              id: undefined,
              references_id: testObj.identifierGenerator.generateId(),
              service_name: SyncModel.SERVICE_SYNC_USER,
              status: SyncModel.STATUS_ERROR,
            },
          ];
        },
      ...
      expect(result[0]).to.be.instanceOf(SyncModel).and.includes({
        id: undefined,
        referencesId: testObj.identifierGenerator.generateId(),
        serviceName: SyncModel.SERVICE_SYNC_USER,
        status: SyncModel.STATUS_ERROR,
        insertDate: null,
        updateDate: null,
      });
